    ...
    <div class="w-full bg-white flex justify-center">
      <div class="container">
        <div class="w-full flex flex-col md:flex-row justify-center">
          <div class="w-full res-nav-wrapper md:w-3/12 flex flex-col bg-brand-gray text-white pt-14 pb-10 md:pt-48 px-4 md:px-0 relative">
              ...
              <li class="w-full tab-b mb-7 cursor-pointer" :class="isOpen(index) ? 'open' : ''" v-for="(category, index) in $page.allCategory.edges" :key="index">
                ...
                <div class="w-full" v-if="category.node.belongsTo && category.node.belongsTo.edges.length > 0">
                  <ul class="list-none w-full">
                    <li class="w-full flex justify-between py-3 px-2 border-brand-gray-light border-b last:border-none hover:text-brand-lime" @click="selectPage(sub_category.node.id, index)" v-for="(sub_category, index) in category.node.belongsTo.edges" :key="index">
...
.res-nav-wrapper::after {
  content: "";
  width: 50vw;
  @apply h-full absolute right-0 bg-brand-gray top-0 z-0 border-b-2 border-b-brand-gray-light;
}
.res-hero-lead-wrapper::after {
  content: "";
  width: 50vw;
  @apply h-full absolute right-0 bg-brand-lime top-0 z-0;
}
</style>
