<template>
  <Layout>
      ...
      <div class="container">
        <div class="w-full flex flex-col md:flex-row md:items-stretch relative">
          <div class="w-full md:w-4/12 pt-12 md:pb-14 md:pt-[74px] flex flex-col md:pr-20">
            <h4 class="text-[36px] md:text-[50px] leading-[60px] tracking-[-1.08px] md:tracking-[-1px] font-normal mt-[31px] relative flex items-start before:bg-brand-lime before:w-[24px] before:h-[24px] before:inline before:mt-[11px] before:mr-6 before:flex-shrink-0">{{ $page.pages.openings_section.header }}</h4>
          </div>
          <div class="w-full md:w-8/12 flex flex-col md:pt-[74px] md:border-l border-brand-gray-lighter md:pl-[60px]">
            <div class="w-full flex flex-col tab cursor-pointer transition-all" :class="isOpen(index) ? 'open' : ''" v-for="(category, index) in $page.pages.openings_section.openings.categories" :key="index">
              <div class="flex justify-between py-3 md:py-7 border-b border-brand-gray items-center" @click="openTab(index)">
                <p class="text-3xl md:text-[38px] leading-normal tracking-normal">{{ category.title }}</p>
              ...
              <ul class="list-none w-full">
                <li class="w-full flex justify-between" v-for="(job, index) in category.jobs" :key="index">
                  <g-link :to="job.link" class="text-xl md:text-[25px] leading-normal tracking-[-0.03px] text-brand-gray-light underline py-3">{{ job.title }}</g-link>
                  <span class="text-[25px] leading-normal tracking-[-0.03px] text-[#A6AFBC] py-3">{{ job.location }}</span>
                </li>
              </ul>
            </div>
        ...
        <div class="w-full flex flex-col md:flex-row md:items-stretch relative">
          <div class="w-full md:w-4/12 pt-16 pb-4 md:py-20 md:pt-[74px] flex flex-col">
            <h4 class="text-[36px] md:text-[50px] leading-[50px] md:leading-[60px] tracking-[-1.08px] md:tracking-[-1px] font-normal">
          ...
          <div class="w-full md:w-8/12 flex flex-col md:pt-[74px] pb-14 md:pb-32 md:border-l border-brand-gray-lighter md:pl-[60px]">
            <p class="text-base md:text-[25px] leading-[26px] md:leading-[37px] md:tracking-[-0.5px] md:max-w-[644px]">{{ $page.pages.openings_section.body }}</p>
          </div>
        </div>
      </div>
    </div>
    <div class="w-full before:hidden md:before:block before:bg-brand-lime before:w-1/2 before:h-full before:absolute before:top-0 before:left-0 before:-z-10 relative">
      <div class="container px-0 md:px-4 flex flex-col md:flex-row z-10 md:items-stretch">
        <div class="w-full md:w-1/2 relative py-12 md:pt-16 md:pb-28 md:pr-16 bg-brand-lime md:bg-transparent px-4 md:px-0">
          <div class="w-full flex justify-between items-center">
          ...
          <div class="w-full mt-10 md:mt-24">
            <ul class="list-none">
              <li class="text-3xl leading-loose md:text-[50px] md:leading-[70px] tracking-[-1px] font-normal flex flex-col justify-start group value-item cursor-pointer" v-for="(benefit, index) in $page.pages.benefits_section.benefits" :key="index" :class="isActive(index) ? 'active' : ''" @click.capture="activate(index)">
